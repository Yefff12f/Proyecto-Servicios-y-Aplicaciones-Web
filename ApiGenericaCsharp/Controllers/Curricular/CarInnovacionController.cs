...
namespace ApiProyecto.Controllers.Curricular
{
    ...
    public class CarInnovacionController : BaseController
    {
        ...
        {
            var lista = await _context.CarInnovaciones.ToListAsync();
        ...
        {
            var item = await _context.CarInnovaciones.FindAsync(id);
            if (item == null) return NotFound();
            return Ok(item);
        }

        [HttpPost]
        public async Task<ActionResult> Post(CarInnovacion item)
        {
            _context.CarInnovaciones.Add(item);
            await _context.SaveChangesAsync();
            return Ok(new { mensaje = "Registro creado correctamente" });
        }

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, CarInnovacion item)
        {
        ...
        [HttpPut("{campo}/{valor}")]
        public async Task<IActionResult> PutPorCampo(string campo, string valor, CarInnovacion item)
        {
        ...
        public async Task<IActionResult> Delete(int id)
        {
            var item = await _context.CarInnovaciones.FindAsync(id);
            if (item == null) return NotFound();
            _context.CarInnovaciones.Remove(item);
            await _context.SaveChangesAsync();
